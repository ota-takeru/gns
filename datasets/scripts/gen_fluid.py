...
def _generate_split(
    ...
    def _parse_idx(path: Path) -> Optional[int]:
    ...
    scenes: Dict[int, Tuple[np.ndarray, np.ndarray, Optional[Dict[str, Any]]]] = {}

    existing = []
    for path in sorted(split_dir.glob("scene_*.npz")):
        idx = _parse_idx(path)
        if idx is None or idx >= num_scenes:
            continue
        with np.load(path, allow_pickle=False) as data:
            ...
            if meta_arr is not None:
                try:
                    ...
                    meta = None
        scenes[idx] = (positions, particle_types, meta)
        existing.append(idx)

    if existing:
        existing_str = ", ".join(map(str, sorted(existing)))
        print(
            f"\n[{cfg.name}] Found {len(existing)} existing {split} scene(s): {existing_str}. Resuming..."
        )
    else:
    ...
    ordered_indices = sorted(scenes.keys())
    trajectories: List[np.ndarray] = [scenes[i][0] for i in ordered_indices]
    particle_types_list: List[np.ndarray] = [scenes[i][1] for i in ordered_indices]
    last_meta = next((scenes[i][2] for i in reversed(ordered_indices) if scenes[i][2]), None)

    if len(trajectories) < num_scenes:
        missing = num_scenes - len(trajectories)
        raise RuntimeError(f"[{cfg.name}] Missing {missing} {split} scene(s) after resume.")
...
def run_generation(
    ...
    cases = dataset_cfg.cases
    if target_cases:
        available = {c.name for c in cases}
        missing = [c for c in target_cases if c not in available]
        if missing:
            raise ValueError(f"Invalid case name(s): {missing}. Available: {sorted(available)}")
        cases = [c for c in cases if c.name in target_cases]
        print(f"\n=== Generating {len(cases)} case(s): {', '.join(target_cases)} ===")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Generate only the specified case names (default: all cases in config).",
    )
...
def main() -> None:
    args = parse_args()
    run_generation(args.config, args.output_root, target_cases=args.cases)
