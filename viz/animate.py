...
def animate_points(pos, radius=None, save_mp4="rollout.mp4", fps=30, lim=None):
    """
    ...
    scat = ax.scatter(
        pos[0, :, 0], pos[0, :, 1], s=20 if radius is None else (radius * 100) ** 2
    )
    ax.set_aspect("equal")
    if lim is None:
    ...
    try:
        ...
        ani.save(save_mp4, writer=writer, dpi=150)
    except Exception:
        # ffmpeg無い環境ではHTML5アニメを返す
        from matplotlib.animation import HTMLWriter

        ani.save(save_mp4.replace(".mp4", ".html"), writer=HTMLWriter(fps=fps))
    plt.close(fig)
