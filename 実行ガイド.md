...
最短で学習がうまく進みそうかを確認するためのプリセットを用意しています。
...
# 1コマンドで短時間学習→直後に validation まで実行
uv run quick-check
...
`config_quick.yaml` は 200 ステップ・バッチ2・50ステップごとに軽量 validation を行う設定です。
モデルは `models/quick/` に保存され、本番用の `models/` とは分離されます。

### 3.3 研究ループ向けプリセット（tiny/dev/full）

以下のプリセットで「Inner/Mid/Outer」ループを素早く回せます。

```bash
# Inner（2–10分）：極小データ＆軽量モデルで one-step loss を見る
uv run tiny-train          # 学習（~数分）
uv run tiny-valid          # 直近モデルで one-step 検証

# Mid（15–60分）：軽め設定で粗い○×を付ける（必要なら3シード）
uv run dev-train
uv run dev-valid
uv run dev-3seeds          # 3シード連続実行（時間が許せば）

# Outer（数時間〜一晩）：本命設定で長期 rollouts
uv run full-train
uv run full-valid          # latest で評価（rollout 可視化は 4 章）
```

主な時短ノブ（設定ファイルに反映済み）
- データ量: `train_scenes`, `valid_scenes`（tiny/dev は上限あり）
- 近傍: `connectivity_radius` を縮小
- 計算段数: `nmessage_passing_steps`, `latent_dim`, `mlp_hidden_dim`
- 精度: `amp: fp16`（混合精度）
- I/O: `num_workers`, `prefetch_factor`, `pin_memory`, `persistent_workers`

早期シグナルとキルスイッチ（実装済み）
- one-step valid を `validation_interval` ごとに計測
- `early_stop_check_step` 時点で初期より `early_stop_required_improvement` 以上の改善が無ければ中断
- 出力ログに `grad_norm` と `it/s` を表示
- 学習終了時に `models/*/exp_log.csv` に最低限の指標を1行追記

メモの最小化
- 各 run の末尾に `exp_log.csv` に行追加（commit/cfg/seed/step/損失など）。
- 追加で可視化が必要なら 4 章の手順で rollout を出力・可視化してください。
