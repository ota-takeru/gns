...
model_path: ./models/
tensorboard_enable: false     # TensorBoard ロギングを有効化するか（デフォルト false）
...
`method: hamiltonian_sph` を使う場合は、`method_options.hamiltonian_sph.hamiltonian_net` で保存枝の特徴量モードを YAML から選択できます。`use_sph_features` を `true` にすると SPH 由来特徴（Mode B）、`false` で距離・質量中心の最小構成（Mode A）。`use_velocity_in_H` は相対速度を入れるかどうかの切替で、学習と推論の YAML を揃えれば同じ挙動で実行できます。
...
TensorBoard を完全に無効化したい場合は `tensorboard_enable: false` を指定してください（デフォルトも false）。有効化するには true に変更し、必要に応じて `tensorboard_log_dir` やポートを設定します。
...
3. 可視化（visualize_rollout.py）  
   - 既定: MP4 出力  
   - `--html` を付けて実行した場合は HTML 出力
...
# 既定は MP4 出力（例: rollouts/rollout_ex0.mp4）
...
### 5.1 可視化ファイルの確認（MP4 / HTML）

`visualize_rollout.py` の既定出力は MP4 です。特定のパスを指定しない場合、入力 pkl と同じ場所に `.mp4` が作成されます。`--format html` や `--html` オプションを付けると HTML を出力します（`run_inference.sh` は後方互換のため HTML を生成します）。

```
rollouts/
  ├── rollout_ex0.mp4   # 既定
  ├── rollout_ex1.mp4
  ├── rollout_ex0.html  # --format html を指定した場合
  ├── rollout_ex1.html
  └── ...
```

MP4 は任意の動画プレーヤーで再生できます。HTML で保存した場合はブラウザで開くとインタラクティブに確認できます。
