...
class LearnedSimulator(nn.Module):
    def __init__(self,
                ...
                node_in: int, #ノード入力特徴量の次元数
                edge_in: int, #エッジ入力特徴量の次元数
        ...
        self._particle_type_embedding = nn.Embedding(nparticle_types, particle_type_embedding_size) #粒子タイプの埋め込み層
    ...
    def _compute_graph_connectivity(self,
        ...
        batch_ids = torch.cat(
            [torch.LongTensor([i for _ in range(n)])
             for i, n in enumerate(nparticles_per_example)]).to(self._device)    #バッチIDを生成 (0,0,0,...,1,1,1,...)
        edge_index = radius_graph(
            node_position, r=radius, batch=batch_ids, loop=add_self_edges, max_num_neighbors=128) #近傍グラフを計算
    ...
    def _encoder_preprocessor(self,
        ...
        nparticles_per_example: torch.tensor, #[粒子数, 粒子数, ...]。複数のシーンを同時に扱うため、近傍計算の際にシーンを区別できるようにするため。
        particle_types: torch.tensor, #[N],
        material_property: torch.tensor = None
        ): #node_features、edge_features、ede_indexを生成。

    #------node_featuresの計算------
        node_features = []
        nparticles = position_sequence.shape[0] #粒子数
        most_recent_position = position_sequence[:, -1] #最新の位置
        velocity_sequence = self.time_diff(position_sequence)  #速度列
    ...
    #------edge_featuresの計算------
        senders, receivers = self._compute_graph_connectivity(most_recent_position, nparticles_per_example, self._connectivity_radius)
        normalized_relative_displacements = (most_recent_position[senders, :] - most_recent_position[receivers, :]) / self._connectivity_radius #[E, D]
        normalized_relative_distances = torch.norm(normalized_relative_displacements, dim=-1, keepdim=True) #[E, 1]

        edge_features = torch.cat(
            [normalized_relative_displacements,
             normalized_relative_distances], dim=-1)
    ...
    def _decoder_postprocessor(self, 
        ...
        position_sequence: torch.tensor): #正規化された加速度から次の位置を計算
    ...
    def _inverse_decoder_postprocessor(self, next_position: torch.tensor, position_sequence: torch.tensor): #次の位置、ノイズ付き位置列から正規化された加速度を計算
    ...
    def predict_positions(self,
        ...
        material_property: torch.tensor = None): #次の位置を予測
        if material_property is not None:
        ...
        predicted_normalized_acceleration = self._encode_process_decode(
            node_features, edge_index, edge_features)   #正規化された加速度を予測
        predicted_positions = self._decoder_postprocessor( 
            predicted_normalized_acceleration, current_positions) #次の位置を計算
    ...
    def predict_accelerations(self,
        ...
        with torch.no_grad():
            next_position_adjusted = next_positions + position_sequence_noise[:, -1] #正確な教師データを用意するために、next_positionsにノイズを加算することでノイズの影響を相殺させる。
            target_normalized_acceleration = self._inverse_decoder_postprocessor(
                next_position_adjusted, noisy_position_sequence) #正規化された目標加速度を計算
