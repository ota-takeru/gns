...
GNSは物質の運動を予測する手法です。
物質を粒子の集まりとしてとらえ、粒子の状態を入力として次の時刻の粒子の状態をグラフネットワークというニューラルネットワークによって予測します。
...
作成したグラフに対してグラフネットワークを適用し各粒子の次の時刻の加速度を予測します。
グラフネットワークは入力されたグラフに対して各ノードの近傍のノードから情報を集約し、ノードを更新します。
右下の図では、ノードx1に対して、近傍のノードに重みwをかけて足すことで、ノードの値をz1に更新しています。
これはメッセージパッシングと呼ばれます。このメッセージパッシングを複数回繰り返すことで、粒子の位置や速度による相互作用を反映することができます。
グラフネットワークではこの近傍のノードの情報を集約する関数がニューラルネットワークにおける重みをもちます
出力された加速度が正しい加速度に近づくようにこの重みを更新することで、学習を進めていきます。

---

実際にGNSを用いて流体を学習し再現してみます。
SPH法という流体の生成方法を用いて学習データを作成しました。
学習データのシミュレーションの始めから終わりまでの時間の長さが200ステップの物を用意しました。
また、学習データの数は100サンプル用意しました。1つのサンプルには流体のシミュレーションの始めから終わりまでの情報が入っており、各サンプルの初期条件を少し変えています。
...
まずは学習データの始めから終わりまでの時間の長さを変えて実験します。
100,200,400,800ステップで試しました。
結果は左下の図です。
横軸が時間変化で、縦軸が距離の誤差を表しています。
学習データの時間の長さを変えることで、一貫した誤差の変化は見られませんでした。
